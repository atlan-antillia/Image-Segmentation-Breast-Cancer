...
# 
# 2 Resize each image to 256x256
#
...
def create_augmented_master_256x256(input_dir, output_dir, image_format=".jpg", augment_all=True):
  #class_dirs = ["benign", "malignant"]
  class_dirs = ["malignant"]
...
# 1 Resize an original image to 256x256 size,
...
def create_resized_rotated_flipped_files(images_dir, output_dir, class_dir, mask_files, image_format, target): 
  ...
  for mask_file in mask_files:
    ...
    # 1 Resize the mask_img to 256x256
    ...
    # 2 Resize the non_mask_img to 256x256
...
def create_resized_files(images_dir, output_dir, class_dir, mask_files, image_format, target): 
  ...
  for mask_file in mask_files:
    ...
    # 3 Resize the non_mask_img to 256x256
    resized_non_mask_img  = cv2.resize(non_mask_img, (W, H), interpolation = cv2.INTER_LANCZOS4)
    save_non_mask_filename = basename.split("_")[0] + image_format
...
if __name__ == "__main__":
  try:
    ...
    output_dir = "./BUSI_augmented_master_256x256"
    ...
    # create BUS_augmented_master_256x256 dataset train, test
    # from the orignal Dataset_BUSI_with_GT .
    create_augmented_master_256x256(input_dir, output_dir, image_format=".jpg", augment_all=False)
